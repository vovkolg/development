...
NET SESSION > NUL 2>&1
IF %ERRORLEVEL% NEQ 0 (
	ECHO Uninstalling the Predix Machine Service requires system administrator privileges. Please try again with an elevated command prompt.
	EXIT /B
)

SET MSI=%~dp0
MSIEXEC /x %MSI%\PredixMachineSetup.msi /qn /L pmserviceuninstall.log
IF %ERRORLEVEL% NEQ 0 (
	ECHO Predix Machine Service removal failed with an error. View the pmserviceinstall.log for more details.
) ELSE (
	ECHO Predix Machine Service was removed successfully.
)