...
VM_ARGS="-Dmbs.log.custom=com.prosyst.util.log.buffer.BufferedLogger -Dmbs.log.useEventThread=false -Dmbs.log.file.entriesThreshold=0 -Dorg.osgi.framework.bootdelegation=org.bouncycastle.* -Djava.security.egd=file:///dev/urandom"
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/bcpkix-jdk15on-1.52.jar" ]; then
    EXTRA_CP="$EXTRA_CP:${PREDIX_MACHINE_HOME}/machine/bundles/bcpkix-jdk15on-1.52.jar"
fi

# Boot delegation requires this on an absolute path
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/bcprov-jdk15on-1.52.jar" ]; then
    EXTRA_CP="$EXTRA_CP:${PREDIX_MACHINE_HOME}/machine/bundles/bcprov-jdk15on-1.52.jar"
fi
...
generate_password()
{
    KEYPASS=`head -c 500 /dev/urandom | LC_CTYPE=C tr -dc "a-zA-Z0-9-_+{}|:<>?" | head -c $1`
}
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.machineadapter-opcua-16.2.2.jar" ]; then
    OPCUA_KEYSTORE_PATH=security/opcua_keystore.jks
fi
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.opcua-server-16.2.2.jar" ]; then
