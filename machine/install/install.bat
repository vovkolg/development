...
:PROSYSTKEY
	REM Prosyst has a limited key that must not be replaced by the installation
	CALL :WRITECONSOLELOG "Backing up Prosyst limited key."
	XCOPY /S /E /G /C /Y machine\bin\vms\domain.crp "%UPDATEDIR%"\machine\bin\vms\domain.crp>>"%LOG%" 2>&1
	IF !ERRORLEVEL! NEQ 0 (
		SET MESSAGE=Prosyst limited key could not be inserted into the update package.
		SET ERRORCODE=4
		SET STATUS=failure
		CALL :FINISH
	)
